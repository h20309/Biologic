...
class KBIO_api:
    def GetLibVersion(self):
        try:
            version = c_buffer(32)
            self.BL_GetLibVersion(*version.parm)
    ...
    def GetChannelInfo(self, id_, ch):
        ...
        try:
            self.BL_GetChannelInfos(id_, ch - 1, info)
    ...
    def LoadFirmware(self, id_, channels, firmware, fpga, force=True):
        ...
        # shift through results and print message in case of error
        for ch, r in enumerate(results):
            error = self.Error(r)
            error.check(f"LoadFirwmare on {ch+1}", abort=False)

    def LoadFlash(self, id_, firmware):
        return self.BL_LoadFlash(
            id_,
            firmware.encode() if firmware else None,
            True,  # display progress bar
        )

    def GetHardConf(self, id_, ch):
        conf = self.HardwareConf()
        self.BL_GetHardConf(id_, ch - 1, conf)
        return conf

    def SetHardConf(self, id_, ch, cnx, mode):
    ...
    def GetErrorMsg(self, code):
        ...
        self.BL_GetErrorMsg(code, *message.parm)
        return message.value
    ...
    def ConvertChannelNumericIntoSingle(self, vi, board_type):
        """Convert the vi word (32b) into a float depending on channel."""
        vf = c_float()
        self.BL_ConvertChannelNumericIntoSingle(vi, vf, board_type)
        return vf.value

    def ConvertTimeChannelNumericIntoSeconds(self, data, time_base, board_type):
        """Extract time from data"""
        time = c_double()
        self.BL_ConvertTimeChannelNumericIntoSeconds((c_uint32 * len(data))(*data), time, time_base, board_type)
        return time.value

    def GetChannelBoardType(self, connection_id, channel):
        """Get board type of a channel"""
        board_type = c_uint32()
        self.BL_GetChannelBoardType(connection_id, channel - 1, board_type)
        return board_type.value

    # ==============================================================================#

    def FindEChemDev(self):
        serialized = c_buffer(65536, "UTF16")
        nb_devices = c_uint32()
    ...
    def FindEChemEthDev(self):
        serialized = c_buffer(32768, "UTF16")
        nb_devices = c_uint32()
    ...
    def SetConfig(self, target_ip, new_ip=None, netmask=None, gateway=None):
        new_config = ""
        if new_ip:
        ...
        return self.BL_SetConfig(target_ip.encode(), new_config.encode())
    ...
    def _parse_device_serialization(cls, nb_devices, serialized):
        ...
        warn_diff("unexpected nb of devices", nbs)
    ...
    class DeviceInfo(KBIO.DeviceInfo):
        ...
        def __str__(self):
            ...
            fragments.append(f"{self.model} {self.RAMSize}MB, CPU={self.CPU}, {pp_plural(channels, 'channel')}, {pp_plural(slots, 'slot')}")
            fragments.append(
                f"Firmware: v{self.FirmwareVersion/100:.2f} {self.FirmwareDate_yyyy}/{self.FirmwareDate_mm}/{self.FirmwareDate_dd}"
            )

            cnx = self.NbOfConnectedPC
            fragments.append(f"{pp_plural(cnx, 'connection')}" f", HTdisplay {'on' if self.HTdisplayOn else 'off'}")
        ...
        def __str__(self):
            ...
            if self.has_no_firmware:
                ...
                if self.NbAmps:
                    ...
                    fragments.append("no amplifiers")
                ...
                if memsize:
                    fragments.append(f"Memory: {self.MemSize/1024:.1f}KB" f" ({(self.MemFilled/self.MemSize*100.):.2f}% filled)")
                else:
                ...
                fragments.append(f"{self.firmware} (v{vstr}), " f"FPGA ({self.XilinxVersion:04X})")
    ...
    ecl_api = [
        ...
        ("BL_Connect", [c_char_p, c_uint8, c_int32_p, KBIO.DEVICE_INFO], c_int32),
        ...
        ("BL_LoadTechnique", [c_int32, c_uint8, c_char_p, KBIO.EccParams, c_bool, c_bool, c_bool]),
        ...
        ("BL_GetData", [c_int32, c_int8, KBIO.DataBuffer, KBIO.DATA_INFO, KBIO.CURRENT_VALUES]),
        ...
        ("BL_ConvertTimeChannelNumericIntoSeconds", [c_uint32_p, c_double_p, c_float, c_uint32]),
        ("BL_LoadFlash", [c_int32, c_char_p, c_bool]),
    ]
    ...
    blfind_api = [
        ...
        ("BL_SetConfig", [c_char_p, c_char_p], c_int32),
    ]
