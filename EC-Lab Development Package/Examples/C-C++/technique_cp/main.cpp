...
#define TIMEOUT     5
#define TECHNIQUE   "cp4.ecc" // Note: use ocv.ecc for VMP3 serie and ocv5.ecc for VMP300-P
...
TEccParams_t* get_params()
{
    ...
    // Step #1
    status |= BL_DefineSglParameter( "Current_step", 0.02, 0, &params->pParams[0] ); // I0 (A)
    status |= BL_DefineBoolParameter( "vs_initial", false, 0, &params->pParams[1] ); // vs. init
    status |= BL_DefineSglParameter( "Duration_step", 2.0, 0, &params->pParams[2] ); // Step duration (s)
    // Step #1
    status |= BL_DefineSglParameter( "Current_step", -0.01, 1, &params->pParams[3] ); // I1 (A)
    status |= BL_DefineBoolParameter( "vs_initial", false, 1, &params->pParams[4] );  // scan to E1 s. init
    status |= BL_DefineSglParameter( "Duration_step", 2.0, 1, &params->pParams[5] );  // Step duration (s
    // Step #2
    status |= BL_DefineSglParameter( "Current_step", 0.005, 2, &params->pParams[6] ); // I2 (A)
    status |= BL_DefineBoolParameter( "vs_initial", false, 2, &params->pParams[7] );  // vs. init
    status |= BL_DefineSglParameter( "Duration_step", 2.0, 2, &params->pParams[8] );  // Step duration (s)
    // others
    status |= BL_DefineIntParameter( "Step_number", 2, 0, &params->pParams[9] );              // step number
    status |= BL_DefineIntParameter( "N_Cycles", 1, 0, &params->pParams[10] );                // cycle Nc time
    status |= BL_DefineSglParameter( "Record_every_dE", 0.1, 0, &params->pParams[11] );       // record every dE (V)
    status |= BL_DefineSglParameter( "Record_every_dT", 0.1, 0, &params->pParams[12] );       // or every dT (s)
    status |= BL_DefineIntParameter( "I_Range", KBIO_IRANGE_100mA, 0, &params->pParams[13] ); // I Range
    status |= BL_DefineIntParameter( "Bandwidth", KBIO_BW_7, 0, &params->pParams[14] );       // bandwidth
...
void process_data( int32_t connection_id )
{
    ...
    bool             Running     = true;
    int              SamplesRead = 0;
    std::ofstream    outputFile( "cp_example.csv" );
