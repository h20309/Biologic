...
METHOD_SCHEMAS: Dict[str, List[Dict[str, Any]]] = {
    ...
    "RunPEIS": [
        {"name": "ChannelIndex", "type": "int", "label": "Channel Index", "default": 0, "min": 0, "max": 15},
        {"name": "InitialFrequency_Hz", "type": "float", "label": "Initial Frequency (Hz)", "default": 100000.0, "min": 0.001, "step": 100.0},
        {"name": "FinalFrequency_Hz", "type": "float", "label": "Final Frequency (Hz)", "default": 0.1, "min": 0.001, "step": 0.1},
        ...
        {"name": "AcAmplitude_V", "type": "float", "label": "AC Amplitude (V)", "default": 0.01, "min": 0.000001, "step": 0.001},
        {"name": "OutputFile", "type": "text", "label": "Output File", "default": ""},
    ],
    "RunGEIS": [
        {"name": "ChannelIndex", "type": "int", "label": "Channel Index", "default": 0, "min": 0, "max": 15},
        ...
        {"name": "AcAmplitude_A", "type": "float", "label": "AC Amplitude (A)", "default": 0.0001, "min": 0.0000001, "step": 0.0001},
        {"name": "OutputFile", "type": "text", "label": "Output File", "default": ""},
    ],
    "Charge": [
        {"name": "ChannelIndex", "type": "int", "label": "Channel Index", "default": 0, "min": 0, "max": 15},
        ...
        {"name": "RecordInterval_s", "type": "float", "label": "Record Interval (s)", "default": 1.0, "min": 0.1, "step": 0.1},
        {"name": "OutputFile", "type": "text", "label": "Output File", "default": ""},
    ],
    "Discharge": [
        {"name": "ChannelIndex", "type": "int", "label": "Channel Index", "default": 0, "min": 0, "max": 15},
...
async def browse_opcua_object(object_node_id: str) -> Dict[str, Any]:
...
async def read_node_values(nodes_config: Dict[str, str]) -> Dict[str, Any]:
    client = Client(url=SERVER_URL)
...
async def call_biologic_method(method_name: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    client = Client(url=SERVER_URL)
...
def refresh_dashboard() -> None:
    st.session_state.monitor_snapshot = asyncio.run(read_node_values(resolve_monitor_nodes()))
    st.session_state.biologic_snapshot = asyncio.run(read_node_values(BIOLOGIC_OUTPUT_NODES))
...
def update_chart(values: Dict[str, Any], chart_container) -> None:
    ...
    if st.session_state.x_axis_node:
        ...
        if st.session_state.data_history.get("ElapsedTime"):
    ...
    for index, field in enumerate(schema):
        ...
        with column:
            if field_type == "int":
                ...
                payload[field["name"]] = st.text_input(field["label"], value=str(field.get("default", "")), key=field_key)

    return payload


def render_method_form(method_name: str) -> None:
    ...
    if submitted:
        ...
        result = asyncio.run(call_biologic_method(method_name, normalized_payload))
...
def display_state_info(state_data: Dict[str, Any], chart_container, data_container) -> None:
...
st.title("🔌 Biologic OPC UA Console")
...
with control_tab:
    ...
    for tab, method_name in zip(method_tabs, METHOD_SCHEMAS.keys()):
        with tab:
            render_method_form(method_name)
    ...
    with st.expander("Browse OPC UA Object", expanded=True):
        ...
        with browse_col2:
            ...
            if st.button("Browse", use_container_width=True):
                browse_result = asyncio.run(browse_opcua_object(object_node_id))
                if browse_result.get("success"):
                    st.session_state.available_nodes = browse_result["variables"]
                    st.success(f"Found {browse_result['count']} variables")
                else:
                    st.error(f"Browse failed: {browse_result.get('error')}")
        ...
        if st.session_state.available_nodes:
            ...
            st.session_state.selected_nodes = [st.session_state.available_nodes[i] for i in selected_indices]
...
with st.expander("Instructions", expanded=False):
    st.markdown(
        ...
        - Use Refresh Dashboard Now if you need to force a full read after a long-running test.
        """
    )
